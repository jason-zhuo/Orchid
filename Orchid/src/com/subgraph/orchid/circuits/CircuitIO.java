...
public class CircuitIO implements DashboardRenderable {
	// zhuo added:
	public static Object writeFilelock = new Object();
	public static Object recievelock = new Object();

	private static final Logger logger = Logger.getLogger(CircuitIO.class
			.getName());
	...
	// zhuo add:
	File Scanresult = new File("D://destroy_result");
	...
	Cell receiveControlCellResponse() {
		try {
			final long timeout = getReceiveTimeout();
			return controlCellResponseQueue
	...
	private long remainingBuildTime() {
		final long elapsed = circuit.getStatus()
				.getMillisecondsElapsedSinceCreated();
	...
	// zhuo add:
	public void WriteResponseToFile(String msg) {
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(Scanresult,
					true));
			bw.write(msg);
			bw.flush();
		} catch (IOException e) {
	...
	/*
	 * This is called by the cell reading thread in ConnectionImpl to deliver
	 * control cells associated with this circuit (CREATED, CREATED_FAST, or
	 * DESTROY).
	 */
	void deliverControlCell(Cell cell) {
		if (cell.getCommand() == Cell.DESTROY) {
			// System.out.println("DESTORY recieve!");
			String msg = "DESTORY recieve!\n";

			synchronized (writeFilelock) {
				isReceived = true;
				WriteResponseToFile(msg);
				writeFilelock.notifyAll();				
			}
...
//		logger.fine("DESTROY cell received ("
//				+ CellImpl.errorToDescription(reason) + ") on " + circuit);
		System.out.println("DESTROY cell received ("
		+ CellImpl.errorToDescription(reason) + ") on " + circuit);
		destroyCircuit();
	}
	...
	void deliverRelayCell(Cell cell) {
		...
		case RelayCell.RELAY_TRUNCATE:
			...
			throw new TorException(
					"Unexpected 'forward' direction relay cell type: "
							+ relayCell.getRelayCommand());
		}
	}
	...
	private void processRelayDataCell(RelayCell cell) {
		if (cell.getRelayCommand() == RelayCell.RELAY_DATA) {
			...
			if (cell.getCircuitNode().considerSendingSendme()) {
				final RelayCell sendme = createRelayCell(
						RelayCell.RELAY_SENDME, 0, cell.getCircuitNode());
		...
		synchronized (streamMap) {
			...
			// It's not unusual for the stream to not be found. For example, if
			// a RELAY_CONNECTED arrives after
			// the client has stopped waiting for it, the stream will never be
			// tracked and eventually the edge node
	...
	RelayCell createRelayCell(int relayCommand, int streamId,
	...
	void sendRelayCellTo(RelayCell cell, CircuitNode targetNode) {
		...
		synchronized (relaySendLock) {
			...
			byte[] digestbyte = targetNode.getForwardDigestBytes();
			cell.setDigest(digestbyte);

			// encryption with forward cell
			for (CircuitNode node = targetNode; node != null; node = node
					.getPreviousNode()) {
	...
	void sendCell(Cell cell) {
		...
		if (!(status.isConnected() || status.isBuilding()))
			return;// 状态错误
		try {
			status.updateDirtyTimestamp();// 脏时间戳
	...
	void sendDestroyCell(int reason) {
		...
		try {
			...
			logger.warning("Connection IO error sending DESTROY cell: "
					+ e.getMessage());
		}
	}
	...
	void destroyCircuit() {
		synchronized (streamMap) {
			...
			circuit.setStateDestroyed();
			CircuitNode d = circuit.getFinalCircuitNode();
		    List<CircuitNode> lsit = circuit.getNodeList();
			System.out
					.println("destory conn with: " + d.getRouter().toString());
			connection.removeCircuit(circuit);

			final List<StreamImpl> tmpList = new ArrayList<StreamImpl>(
					streamMap.values());
	...
	StreamImpl createNewStream(boolean autoclose) {
		synchronized (streamMap) {
			...
			final StreamImpl stream = new StreamImpl(circuit,
					circuit.getFinalCircuitNode(), streamId, autoclose);
	...
	public void dashboardRender(DashboardRenderer renderer, PrintWriter writer,
